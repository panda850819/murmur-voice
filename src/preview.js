...
// ⚡ Bolt Optimization: Cache DOM elements to avoid expensive querySelectorAll calls
...
function updateDictBar() {
  const chips = activeChips.filter(c => !c.classList.contains("added"));
  if (chips.length === 0) {
    hideDictSuggest();
    return;
  }
...
function hideDictSuggest() {
  ...
  const container = dictChips();
  while (container.firstChild) container.removeChild(container.firstChild);
  activeChips = []; // Clear cache to prevent stale references/memory leaks
}
