...
// Cache original text set to avoid redundant tokenization and Set creation
// during high-frequency input events when only the edited text changes.
let lastOriginalText = null;
let cachedOrigSet = null;

function wordDiff(original, edited) {
  if (original !== lastOriginalText) {
    // Apply toLowerCase() before tokenization to avoid mapping over intermediate arrays
