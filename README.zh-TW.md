...
```
語音：  快捷鍵 -> 錄音 (cpal) -> 轉錄 (Whisper) -> LLM 整理 (選用) -> 智慧剪貼簿
翻譯：  Option+T -> 複製選取文字 -> LLM 翻譯 -> 貼上取代原文
```

**每次錄音最多觸發 2 次 API 呼叫**（使用 Groq 時）：一次 Whisper 轉錄，一次 LLM 後處理。翻譯觸發 1 次 API 呼叫。
...
在任何 app 中選取文字，按下 **Option+T**（預設）即可原地翻譯。翻譯後的文字會取代原本的選取內容，並顯示在預覽視窗中。

- 使用 AI 處理中設定的同一個 LLM 供應商（Groq、Ollama 或 Custom）
- 目標語言可在 **設定 > 翻譯 > 目標語言** 中設定
- 翻譯快捷鍵可在 **設定 > 翻譯 > 翻譯快捷鍵** 中自訂
- 預覽視窗會持續顯示，直到你關閉或點擊複製
