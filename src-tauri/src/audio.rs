...
fn resample_linear_into(input: &[f32], ratio: f64, output: &mut Vec<f32>) {
    ...
    // Optimization: Split loop into hot path (safe bounds) and tail path.
    let safe_output_len = if input.len() > 1 {
        (((input.len() - 1) as f64 * ratio).floor() as usize).min(output_len)
    } else {
        0
    };

    // Hot path: guaranteed that src_idx + 1 < input.len()
    for i in 0..safe_output_len {
        let src_pos = i as f64 * inv_ratio;
        let src_idx = src_pos as usize;
        let frac = (src_pos - src_idx as f64) as f32;

        // Optimization: safe bounds, direct array indexing
        let p1 = unsafe { *input.get_unchecked(src_idx) };
        let p2 = unsafe { *input.get_unchecked(src_idx + 1) };

        // Algebraic simplification: p1 * (1.0 - frac) + p2 * frac == p1 + (p2 - p1) * frac
        let sample = p1 + (p2 - p1) * frac;
        output.push(sample);
    }

    // Tail path: boundary conditions handled safely
