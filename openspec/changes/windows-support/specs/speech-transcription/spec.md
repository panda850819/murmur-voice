...
The system SHALL transcribe audio samples using whisper-rs with the ggml-large-v3-turbo model, performing all inference locally on the device. On macOS, the system SHALL use Metal GPU acceleration. On Windows, the system SHALL use CPU inference. When personal dictionary terms are configured, the system SHALL inject them as initial_prompt.
